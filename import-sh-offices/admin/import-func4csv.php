...
if ( isset( $_POST['_wpnonce-is-iu-import-sh-offices-page_import_cn'] ) ) {
    ...
    if (!empty($_FILES['users_csv_cn']['name'])) {
        ...
        $csv_content = array_map( 'str_getcsv', file($filename) );
        ...
        foreach ($csv_content as $index => $row) {
            ...
            if ($post_id > 0) {
                ...
                // Import custom fields/meta data
                foreach ($row as $idx => $col) {
                    if($idx <= 2) {
                        continue;
                /*    } else if(in_array($field_names[$idx], $taxonomy_names)) { */

                        /*
                        // Set the taxonomy value for the workspace post
                        $taxonomy = 'workspace'; // Replace 'your_taxonomy_name' with the actual name of your taxonomy
                        $term = 'workspaces-v4'; // Replace 'your_term_slug' with the slug of the term you want to set

                        // Set the terms for the workspace post
                        ...
                        */
                        ...
                        // continue;
                    } else if($field_names[$idx] == "Image Featured"){
                        // Set the featured image
                        // Add Featured Image to Post
                        // $image_name       = 'avatar5981083818.jpg';
                        // $image_url        = 'http://localhost/sharedoffices/wp-content/uploads/2023/07/avatar5981083818.jpg'; // Define the image URL here

                        // $image_name       = 'avatar5915259602.jpg';
                    ...
                    update_post_meta($post_id, $field_names[$idx], $col); // Assuming the custom field key is in the third column
                ...
                echo '<div style="color: green">'.$index.'. '.$row[0].', '.$row[1].', '.$row[2].' successed. </div><br>';               

            } else {
                // Post was not inserted, handle the error if needed
                echo '<div style="color: red">'.$index.'. '.$row[0].', '.$row[1].', '.$row[2].' failed. </div><br>';               
